...
def load_submission_data_from_db(
    engine, filter_time_before: str = FILTER_TIME_BEFORE, filter_time_after: str = FILTER_TIME_AFTER
) -> pd.DataFrame:
    """
...
def load_market_data_from_db(engine, sql_query: str | None = None) -> pd.DataFrame:
    """
...
def load_event_data_from_db(engine, sql_query: str | None = None) -> pd.DataFrame:
    """
...
def load_data_from_db(
    ...
    logger.info(
        f"Filtering data created before: {filter_time_before} and after: {filter_time_after}"
    )
    ...
    submissions_df = load_submission_data_from_db(engine, filter_time_before, filter_time_after)
...
def process_predictions(
    ...
    logger.info(
        f"Filtered to {len(predictions_df)} predictions with valid event tickers "
    ...
    markets_df["is_valid"] = markets_df.apply(
        lambda row: validate_market_row(row, use_bid_for_odds), axis=1
    )
    initial_market_count = len(markets_df)
    markets_df = markets_df[markets_df["is_valid"]].drop(columns=["is_valid"])
    logger.info(
        f"Filtered to {len(markets_df)} valid market rows (removed {initial_market_count - len(markets_df)})"
    )
    ...
    for event_ticker in valid_event_tickers:
        event_markets = markets_df[markets_df["event_ticker"] == event_ticker].sort_values(
            "created_at"
        )
    ...
    augmented_submissions_df = augmented_submissions_df.sort_values(
        ["event_ticker", "snapshot_time"]
    )
    ...
    augmented_submissions_df = augmented_submissions_df.merge(
        submissions_df[["id", "snapshot_key"]], on="snapshot_key", how="left"
    )
    augmented_submissions_df["submission_id"] = (
        augmented_submissions_df["id"].fillna("").astype(str)
    )
    ...
    predictions_df = predictions_df.merge(
        events_df_indexed[["market_outcome", "category"]],
        left_on="event_ticker",
        right_index=True,
        how="inner",
    )
    ...
    # Further leave only rows in `augmented_submissions_df` whose `submission_id` is in `predictions_output`
    augmented_submissions_df = augmented_submissions_df[
        augmented_submissions_df["submission_id"].isin(predictions_output["submission_id"])
    ]
    logger.info(
        f"Filtered to {len(augmented_submissions_df)} augmented submissions with valid submission_ids"
    )
    ...
    submissions_output = pd.DataFrame(
        {
            ...
            "title": augmented_submissions_df["title"],
