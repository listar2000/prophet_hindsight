...
def first_n_weighting(
    n=1, group_col: list[str] = ["forecaster", "event_ticker"], time_col: str = "round"
):
...
def _simplify_prediction(prediction: dict) -> tuple[np.ndarray, str]:
    ...
    return np.array([prediction_dict[mkt] for mkt in sorted(list(prediction_dict.keys()))]), str(
        rationale
    )
...
def _parse_and_merge_predictions_and_submissions(
    predictions_df: pd.DataFrame, submissions_df: pd.DataFrame, weight_fn
) -> pd.DataFrame:
    """
    Taking both the predictions and submissions dataframes, parse the JSON columns and merge them on the event_ticker and round columns.

    Args:
        predictions_df: DataFrame containing the predictions
        submissions_df: DataFrame containing the submissions
        weight_fn: Function to weight the forecasts

    Returns:
        DataFrame containing the merged forecasts
    """
    predictions_df = predictions_df.copy()
    submissions_df = submissions_df.copy()
    # Parse JSON columns
    predictions_df["prediction"], predictions_df["rationale"] = zip(
        *predictions_df["prediction"].apply(json.loads).apply(_simplify_prediction)
    )
    submissions_df["market_outcome"] = (
        submissions_df["market_outcome"].apply(json.loads).apply(_simplify_market_outcome)
    )
    ...
    submissions_df["odds"], submissions_df["no_odds"] = zip(
        *submissions_df["market_data"].apply(_turn_market_data_to_odds)
    )
    ...
    odds_len, prediction_len, market_outcome_len = (
        merged["odds"].apply(len),
        merged["prediction"].apply(len),
        merged["market_outcome"].apply(len),
    )
    merged = merged[(odds_len == prediction_len) & (odds_len == market_outcome_len)]

    # Add `relative_round` column
    merged["time_rank"] = (
        merged.groupby(["forecaster", "event_ticker"])["round"].rank(ascending=False) - 1
    )
...
class ProphetForecasts:
    ...
    def __init__(
        ...
        raw_submissions_df: pd.DataFrame,
        merged_forecasts_df: pd.DataFrame,
        exclude_forecasters: list[str] | None = None,
    ):
        ...
        if exclude_forecasters is not None:
            merged_forecasts_df = merged_forecasts_df[
                ~merged_forecasts_df["forecaster"].isin(exclude_forecasters)
            ]
            print(
                f"Filtered out {prev_len - len(merged_forecasts_df)} forecasts. Remaining {len(merged_forecasts_df)}."
            )
    ...
    def from_database(
        cls,
        filter_time_before: str | None = None,
        filter_time_after: str | None = None,
        filter_agent_only: bool = False,
        weight_fn: Callable = uniform_weighting(),
        exclude_forecasters: list[str] | None = None,
        ...
        filter_time_before = (
            FILTER_TIME_BEFORE if filter_time_before is None else filter_time_before
        )
        ...
        logger.info("Loading forecasts from database")
        ...
        predictions_output_df, submissions_output_df = process_predictions(
        ...
        merged = _parse_and_merge_predictions_and_submissions(
            predictions_output_df[cls.PREDICTION_COLS],
            submissions_output_df[cls.SUBMISSION_COLS],
            weight_fn,
        )
        return cls(predictions_output_df, submissions_output_df, merged, exclude_forecasters or [])
    ...
    def from_directory(
        ...
        merged = _parse_and_merge_predictions_and_submissions(
            predictions_df[cls.PREDICTION_COLS], submissions_df[cls.SUBMISSION_COLS], weight_fn
        )
