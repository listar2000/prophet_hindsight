...
class EvaluationStage(PipelineStage):
    ...
    def run(self, state: PipelineState) -> PipelineState:
        ...
        """
        from prophet_hindsight.common.forecast import (
            _parse_and_merge_predictions_and_submissions,
        )
        from prophet_hindsight.evaluate.algo import (
        ...
        merged_df = _parse_and_merge_predictions_and_submissions(
            state.predictions_df[ProphetForecasts.PREDICTION_COLS],
            state.submissions_df[ProphetForecasts.SUBMISSION_COLS],
            weight_fn,
        )

        self.logger.info(f"Merged {len(merged_df)} rows")

        # Add market baseline predictions
        self.logger.info("Adding market baseline predictions...")
        merged_with_baseline = add_market_baseline_predictions(merged_df)

        # Compute Brier scores
        self.logger.info("Computing Brier scores...")
        brier_scores_df = compute_brier_score(merged_with_baseline, append=False)

        state.brier_scores_df = brier_scores_df

        self.logger.info(f"Computed Brier scores for {len(brier_scores_df)} predictions")
        return state
